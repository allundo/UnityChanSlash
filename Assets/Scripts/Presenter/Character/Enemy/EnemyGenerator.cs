...
public class EnemyGenerator : Generator<Status>
{
    ...
    /// </summary>
    public virtual IStatus Spawn(EnemyParam param, Vector3 pos, IDirection dir = null, EnemyStoreData data = null)
        => GetInstance(param.prefab).InitParam(param, data).OnSpawn(pos, dir);

    /// <summary>
    /// Instantiate on the object pool specified by 'pool' parameter
    /// </summary>
    public virtual IEnemyStatus Spawn(Transform pool, EnemyParam param, Vector3 pos, IDirection dir, EnemyStatus.ActivateOption option, EnemyStoreData data = null)
