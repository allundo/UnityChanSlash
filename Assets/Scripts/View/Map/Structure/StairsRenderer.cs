...
public class StairsRenderer : StructuresRenderer<StairsControl>
{
    ...
    {
        PlacePrefab(pos, prefabDownStairsN, dir).SetMaterials(floorMaterials.stairs, floorMaterials.wall);
        (map.GetTile(pos) as Stairs).enterDir = dir;
    }
    public void SetUpStairs(Pos pos, IDirection dir)
    {
        PlacePrefab(pos, prefabUpStairsN, dir).SetMaterials(floorMaterials.stairs, null);
        (map.GetTile(pos) as Stairs).enterDir = dir;
    }
}
